...
knitr::opts_knit$set(root.dir = "C:/Users/lina3120/Documents/temp_projects/elasticity_curve/scripts/final_scripts/toy_example")
```
...
library(dataRetrieval); library(dplyr);  library(tidyr); library(tools); library(data.table); library(prism);
library(ggplot2); library(lubridate); library(delayedflow); library(ggpubr); library(rstatix); library(car); library(scales);
library(ggridges); library(rgdal); library(stringr); library(patchwork); library(factoextra);library(cluster); library(plm); library(raster); library(lmtest); library(gtools); library(randomForest); library(varImp); library(smotefamily); library(lemon); library(kableExtra)

```

A series of functions were created to complete this work. These have not yet been converted into a package, and are included here in order to make this file self contained. These are used throughout the subsequent sections. 

```{r functions, echo = TRUE, message=FALSE, warning=FALSE, include = TRUE}
#####################################################################
...
multi_glm <- function(l, forms = formulas, dataframe = df){ #l <- loop_vals[[1]]
  ...
  glm_helper <- function(f){# f<-1
    if(nrow(unique(data[match(substr(ys[f], 1, nchar(ys[f])-2),names(data))])) >5 ){
      ...
      #extract standard errors and P values in sunken env to hide print outs. 
      sink("~temp.txt")
      #se <- summary(res)[,2][2:3]
...
#funciton to cluster data into a number of different clusters so you can look at the geographic distributions of different cluster divisions. 
...
```{r single_site_curves, echo = TRUE, warning=FALSE, include = TRUE}
...
df <- read.csv("./data/processed_flow.csv", colClasses = "character")
...
```{r curve_normalization, echo = TRUE, warning=FALSE, include = TRUE}
...
```{r cluster_final_fit, echo = TRUE, warning=FALSE, include = TRUE}
...
```{r plm_curves, echo = TRUE, warning=FALSE, include = TRUE}
options(scipen=9999)
rm(list = ls()[sapply(ls(), function(x) length(dim(get(x))) > 1)])
...
```{r RF_model, include = TRUE, echo = TRUE}
options(scipen=9999)
rm(list = ls()[sapply(ls(), function(x) length(dim(get(x))) > 1)])
